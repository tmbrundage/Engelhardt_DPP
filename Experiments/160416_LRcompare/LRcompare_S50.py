...
ns = [50,100,150,200,400,800,1600]#[10,20,50,100,200,500,1000,2000,5000]
n_test = 1000
p=40
sparsity=0.50
...
loggingDirectory = 'Logs_P%d_S%d/%s/' % (p,int(100*sparsity),datetime.datetime.fromtimestamp(time.time()).strftime('%Y%m%d_%H%M%S'))
...
for rep in range(repeat):
    for n in ns:
        ...
        DPP_PO = PO.PO(X,y,max_T = 500, GA_max_T = 1000)
        ...
        print "DPP_PO MSE: %f    BETA_LOSS: %f   OPT_LAM: %f" % (DPP_PO_mse, DPP_PO_betaLoss, DPP_PO.c)
        ...
        print "DPP_PO_greedy MSE: %f    BETA_LOSS: %f   OPT_LAM: %f" % (DPP_PO_greedy_mse, DPP_PO_greedy_betaLoss, optLam)
