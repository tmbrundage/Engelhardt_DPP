...
class VL(object):
    ...
    def __init__(self,X,y,**kwargs):
        ...
        self.kappa = int(p*0.1) # Expected cardinality of gamma
        self.alpha = 1.e-1      # Step size
    ...
    def __init__(self,VL,**kwargs):
        ...
        self.logging = dc(VL.loggin)
        self.dir = '/%s/warm_%s/' % (VL.dir,datetime.datetime.fromtimestamp(time.time()).strftime('%Y%m%d_%H%M%S'))
    ...
    def optTheta(self):
        ...
        C = DPPutils.getKDiag(L)
        g = C.dot(self.theta)
        Cbar = np.zeros((self.p,self.p))
        gbar = np.zeros((self.p,1))
        ...
        for step in range(self.max_T):
            ...
            ghat = logPGammaConY(gam, L) * gam  # LINE 6
            Chat = gam.dot(gam.T)               # LINE 7
