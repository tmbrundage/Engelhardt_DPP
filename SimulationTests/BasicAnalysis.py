...
for i in range(setStart,setFinal):
    ...
    def sparsity(predictor):
        sprs = sum(predictor.gamma)[0] / predictor.gamma.shape[0]
        return sprs
    ...
    # LARS2 = dict.fromkeys(NSmeasurements,np.zeros((1,len(dataFolders))))
    # LARS4 = dict.fromkeys(NSmeasurements,np.zeros((1,len(dataFolders))))
    # LARS10= dict.fromkeys(NSmeasurements,np.zeros((1,len(dataFolders))))
    # OLSR  = dict.fromkeys(NSmeasurements,np.zeros((1,len(dataFolders))))
    # RIDGE = dict.fromkeys(NSmeasurements,np.zeros((1,len(dataFolders))))
    # LARS  = dict.fromkeys(measurements.keys(),np.zeros((1,len(dataFolders))))
    # LASSO = dict.fromkeys(measurements.keys(),np.zeros((1,len(dataFolders))))
    # PMG   = dict.fromkeys(measurements.keys(),np.zeros((len(Tthetas),len(dataFolders))))
    # PMS   = dict.fromkeys(measurements.keys(),np.zeros((len(Tthetas),len(dataFolders))))
